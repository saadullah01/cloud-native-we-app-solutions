...
pexpect.run("cp "+sol_dir+"button.svelte "+cmd_dir+"services/web/src/components/ui-elements/")
pexpect.run("cp "+sol_dir+"new-blog-post.svelte "+cmd_dir+"services/web/src/components/")
pexpect.run("git add .", cwd=cmd_dir)
pexpect.run("git commit -m 'Persist new blog post and show success/error message when done'", cwd=cmd_dir)
