...
pexpect.run("git switch -c add-tests", cwd=cmd_dir)
...
pexpect.run("cp "+sol_dir+"index.svelte "+cmd_dir+"services/web/src/routes/blog")
pexpect.run("git add .", cwd=cmd_dir)
pexpect.run("git commit -m 'add end-to-end tests'", cwd=cmd_dir)
ch = pexpect.spawn('git push --set-upstream origin add-tests', cwd=cmd_dir)
