...
# Clone user's repo
pexpect.run(
    "git clone -b add-tailwind-css https://github.com/"+username+"/"+repo_name+".git",
    cwd="/Educative/")
...
pexpect.run("cp "+sol_dir+"contact-card.svelte "+cmd_dir+"services/web/src/components/")
pexpect.run("git add .", cwd=cmd_dir)
pexpect.run("git commit -m 'Use Svelte components to abstract utility classes.'", cwd=cmd_dir)
