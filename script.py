...
pexpect.run("git switch -c add-firestore", cwd=cmd_dir)
pexpect.run("cp "+sol_dir+"template.html "+cmd_dir+"services/web/src/")
pexpect.run("git add .", cwd=cmd_dir)
pexpect.run("git commit -m 'Configure Firestore'", cwd=cmd_dir)
ch = pexpect.spawn('git push --set-upstream origin add-firestore', cwd=cmd_dir)
